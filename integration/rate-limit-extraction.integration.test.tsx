...
describe("Rate Limit Extraction Integration Test", () => {
  ...
  it("should extract rate limit from GitHub API response headers (demo mode)", async () => {
    ...
    expect(data.rateLimit.limit, "Total limit should match header value").toBe(
      5000,
    );

    expect(data.rateLimit.used, "Used count should match header value").toBe(1);
  ...
  it("should update rate limit after each request", async () => {
    ...
    expect(secondResult.data.rateLimit.used, "Used count should increase").toBe(
      1,
    );
  });
