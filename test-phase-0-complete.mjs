...
try {
  ...
  test('Proxy uses GitHub token from env', proxyCode.includes('process.env.GITHUB_TOKEN'));
  ...
  test('Proxy makes GitHub API call', proxyCode.includes('https://api.github.com/graphql'));
} catch (err) {
...
try {
  ...
  test('No direct GitHub API calls', !apolloCode.includes('https://api.github.com/graphql'));
} catch (err) {
...
try {
  ...
  try {
    ...
    log.warn('dist/ not found, skipping bundle check (run npm run build first)');
  }
