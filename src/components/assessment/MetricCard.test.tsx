...
describe('MetricCard', () => {
  ...
  it('does not render breakdown when empty array provided', () => {
    render(<MetricCard title="Activity" score={85} level="High" breakdown={[]} />);
  ...
  it('does not render explain button when onExplainClick not provided', () => {
    ...
    expect(screen.queryByLabelText('Explain Activity score')).not.toBeInTheDocument();
  });
  ...
  it('applies hover effects', () => {
    ...
    const translateCard = container.querySelector('.hover\\:-translate-y-0\\.5');
  ...
  it('renders all level types correctly', () => {
    const levels: Array<'Low' | 'Moderate' | 'High' | 'Strong' | 'Excellent' | 'Exceptional' | 'Minimal'> = [
      'Low', 'Moderate', 'High', 'Strong', 'Excellent', 'Exceptional', 'Minimal'
    ];
  ...
  it('renders multiple breakdown items', () => {
    ...
    render(<MetricCard title="Impact" score={92} level="Exceptional" breakdown={breakdown} />);
