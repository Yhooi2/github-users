/**
 * ExpandableProjectCardGlass Wrapper - Glass UI styled expandable project card
 *
 * Wraps ExpandableProjectCard with Glass UI effects while maintaining
 * the same props interface for backward compatibility.
 *
 * Changes from original:
 * - Card: Already using CardGlass via ui/card.tsx
 * - Badge: Already using BadgeGlass via ui/badge.tsx
 * - Progress: Already using Glass Progress via ui/progress.tsx
 * - Avatar: Already using AvatarGlass via ui/avatar.tsx
 * - Added glass morphism effects to card and sections
 */
...
export interface ExpandableProjectCardGlassProps {
  /** Project data */
...
/**
 * Glass UI styled expandable project card with inline analytics
 *
 * Features:
 * - Glass morphism effects (blur, glow, transparency)
 ...
 * ```
 */
export function ExpandableProjectCardGlass({
  project,
  isExpanded,
  onToggle,
  maxCommits,
}: ExpandableProjectCardGlassProps) {
    ...
    <Card
      ...
      className={cn(
        ...
        isExpanded
          ? [
              "border-primary/30",
              "shadow-[0_0_25px_rgba(var(--primary-rgb),0.15)]",
              "bg-card/80",
            ]
          : [
    ...
    >
              ...
              <span key={lang.name} className="flex items-center gap-1">
                ...
                <span
                  ...
                  style={{
                    ...
                    boxShadow: `0 0 4px ${getLanguageColor(lang.name)}40`,
                  }}
                  aria-hidden="true"
                />
                <span>
      ...
      {isExpanded && (
        <motion.div
          ...
          className="space-y-4 border-t border-border/50 px-3 py-3"
        >
          {/* Description */}
          {project.description && (
          ...
          <div className="flex items-center justify-between gap-3">
            ...
            {/* AI Analytics Button */}
            <AIAnalyticsButtonGlass projectUrl={project.url} />
          </div>

          {/* Your Contribution Section */}
          <ContributionSectionGlass
          ...
          {project.languages && project.languages.length > 0 && (
            <TechStackSectionGlass languages={project.languages} />
          )}
...
/**
 * Your Contribution section with glass styling
 */
interface ContributionSectionGlassProps {
...
function ContributionSectionGlass({
  commitsPercent,
  userCommits,
  totalCommits,
  prsMerged,
  ...
  roleLabel,
  lastActivityDate,
}: ContributionSectionGlassProps) {
  return (
    <section
      aria-labelledby="contribution-heading"
      className="space-y-2 rounded-lg bg-card/30 p-3 backdrop-blur-sm"
    >
...
/**
 * Project Impact section with glass styling
 */
interface ImpactSectionGlassProps {
  stars: number;
  forks?: number;
  teamCount?: number;
}

function ImpactSectionGlass({
  stars,
  forks,
  teamCount,
}: ImpactSectionGlassProps) {
  return (
...
/**
 * Tech Stack section with glass styling
 */
interface TechStackSectionGlassProps {
  languages: LanguageInfo[];
}

function TechStackSectionGlass({ languages }: TechStackSectionGlassProps) {
...
/**
 * Team section with avatars and GitHub link with glass styling
 */
interface TeamSectionGlassProps {
  teamCount?: number;
  topContributors?: TeamMember[];
  projectUrl: string;
  projectName: string;
...
function TeamSectionGlass({
  teamCount,
  topContributors,
  projectUrl,
  projectName,
}: TeamSectionGlassProps) {
  const hasTeam =
    ...
    >
          ...
          {topContributors && topContributors.length > 0 && (
            <div className="flex -space-x-2">
              ...
              {teamCount !== undefined && teamCount > 5 && (
                <div className="flex h-7 w-7 items-center justify-center rounded-full border-2 border-background bg-muted/80 text-xs backdrop-blur-sm">
                  +{teamCount - 5}
                </div>
              )}
            </div>
      ...
      >
        <a
          ...
          className="rounded-md p-1.5 text-muted-foreground transition-colors hover:bg-accent/50 hover:text-primary"
        >
...
/**
 * Health indicator component with glass styling
 */
interface HealthIndicatorGlassProps {
  isArchived: boolean;
  pushedAt: string | null;
}

function HealthIndicatorGlass({
  isArchived,
  pushedAt,
}: HealthIndicatorGlassProps) {
...
/**
 * AI Analytics button (placeholder for future feature) with glass styling
 */
interface AIAnalyticsButtonGlassProps {
  projectUrl: string;
}

function AIAnalyticsButtonGlass(
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  { projectUrl: _projectUrl }: AIAnalyticsButtonGlassProps,
) {
