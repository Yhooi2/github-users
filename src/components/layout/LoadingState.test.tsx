...
describe('LoadingState', () => {
  ...
  describe('list variant count', () => {
    ...
    it('should ignore count for non-list variants', () => {
      const { container: defaultContainer } = render(<LoadingState count={10} />);
      const { container: cardContainer } = render(<LoadingState variant="card" count={10} />);
      const { container: profileContainer } = render(<LoadingState variant="profile" count={10} />);

      // Should not render multiple list items for non-list variants
      // List variant uses .space-y-3 > div with flex gap-4 p-4
      expect(defaultContainer.querySelectorAll('.flex.gap-4.p-4')).toHaveLength(0);
      expect(cardContainer.querySelectorAll('.flex.gap-4.p-4')).toHaveLength(0);
      expect(profileContainer.querySelectorAll('.flex.gap-4.p-4')).toHaveLength(0);
    });
  });
  ...
  describe('variant rendering', () => {
    it('should render different structure for each variant', () => {
      const { container: defaultContainer } = render(<LoadingState variant="default" />);
      const { container: cardContainer } = render(<LoadingState variant="card" />);
      const { container: profileContainer } = render(<LoadingState variant="profile" />);
      const { container: listContainer } = render(<LoadingState variant="list" />);
