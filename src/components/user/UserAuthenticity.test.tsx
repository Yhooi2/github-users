...
describe("UserAuthenticity", () => {
  describe("Rendering", () => {
    ...
    it("should render score and category for medium score", () => {
      vi.spyOn(
        useAuthenticityScoreModule,
        "useAuthenticityScore",
      ).mockReturnValue(mockMediumScore);
      render(<UserAuthenticity repositories={mockRepositories} />);
    ...
    it("should render score and category for suspicious score", () => {
      vi.spyOn(
        useAuthenticityScoreModule,
        "useAuthenticityScore",
      ).mockReturnValue(mockSuspiciousScore);
      const { container } = render(
        <UserAuthenticity repositories={mockRepositories} />,
      );
  ...
  describe("Warning Flags", () => {
    ...
    it("should render flags when present", () => {
      ...
      expect(
        screen.getByText("Low activity in original repositories"),
      ).toBeInTheDocument();
      expect(
        screen.getByText("High fork-to-original ratio"),
      ).toBeInTheDocument();
    });
    ...
    it("should render multiple flags for suspicious score", () => {
      ...
      render(<UserAuthenticity repositories={mockRepositories} />);
      expect(
        screen.getByText("Majority of repositories are forks"),
      ).toBeInTheDocument();
      expect(screen.getByText("Very low engagement")).toBeInTheDocument();
      expect(
        screen.getByText("Suspicious activity pattern"),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("Hook Integration", () => {
    it("should call useAuthenticityScore with repositories", () => {
      const spy = vi
        .spyOn(useAuthenticityScoreModule, "useAuthenticityScore")
        .mockReturnValue(mockHighScore);
  ...
  describe("Edge Cases", () => {
    it("should handle unknown category gracefully", () => {
      ...
      vi.spyOn(
        useAuthenticityScoreModule,
        "useAuthenticityScore",
      ).mockReturnValue(mockUnknownScore);
      const { container } = render(
        <UserAuthenticity repositories={mockRepositories} />,
      );
