...
describe('RecentActivity', () => {
  ...
  describe('Number Formatting', () => {
    it('should format large commit counts with commas', () => {
      const largeCommits = [{ repository: { name: 'big-project' }, contributions: { totalCount: 12456 } }];
  ...
  describe('Edge Cases', () => {
    it('should handle repository with 1 commit', () => {
      const oneCommit = [{ repository: { name: 'test-repo' }, contributions: { totalCount: 1 } }];
    ...
    it('should handle repository with 0 commits', () => {
      const zeroCommits = [{ repository: { name: 'empty-repo' }, contributions: { totalCount: 0 } }];
    ...
    it('should handle long repository names', () => {
      const longName = [
        {
          repository: { name: 'very-long-repository-name-that-might-cause-layout-issues' },
          contributions: { totalCount: 100 },
        },
      ];
      render(<RecentActivity repositories={longName} />);
      expect(screen.getByText('very-long-repository-name-that-might-cause-layout-issues')).toBeInTheDocument();
    });
  });

  describe('Accessibility', () => {
    it('should have aria-hidden on decorative icon', () => {
      const { container } = render(<RecentActivity repositories={mockRepositories} />);
