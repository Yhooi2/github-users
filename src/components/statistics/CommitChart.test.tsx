...
describe('CommitChart', () => {
  ...
  describe('Chart Variants', () => {
    ...
    it('should render chart container when variant is "line"', () => {
      const { container } = render(<CommitChart data={mockData} variant="line" />);

      expect(container.querySelector('[data-slot="chart"]')).toBeInTheDocument();
    });

    it('should render chart container when variant is "bar"', () => {
      const { container } = render(<CommitChart data={mockData} variant="bar" />);

      expect(container.querySelector('[data-slot="chart"]')).toBeInTheDocument();
    });

    it('should render chart container when variant is "area"', () => {
      const { container } = render(<CommitChart data={mockData} variant="area" />);
  ...
  describe('Loading State', () => {
    ...
    it('should show custom loading message', () => {
      ...
      expect(screen.getByText('Fetching data from GitHub...')).toBeInTheDocument();
    });
  ...
  describe('Empty State', () => {
    it('should show empty state when data is empty array', () => {
      ...
      expect(screen.getByText('No commit statistics available for this period.')).toBeInTheDocument();
    });
    ...
    it('should show custom empty title and description', () => {
      ...
      expect(screen.getByText('Start making commits to see statistics')).toBeInTheDocument();
    });
  });
  ...
  describe('State Priority', () => {
    it('should show loading state over error state', () => {
      ...
      expect(screen.getByText('Loading commit statistics...')).toBeInTheDocument();
      expect(screen.queryByText('Failed to load commit statistics')).not.toBeInTheDocument();
    });
