...
describe('LanguageChart', () => {
  ...
  describe('Rendering', () => {
    ...
    it('should display number of languages', () => {
      ...
      expect(screen.getByText(/5 languages across repositories/i)).toBeInTheDocument();
    });
  ...
  describe('Chart Variants', () => {
    ...
    it('should render chart container when variant is "pie"', () => {
      const { container } = render(<LanguageChart data={mockData} variant="pie" />);

      expect(container.querySelector('[data-slot="chart"]')).toBeInTheDocument();
    });

    it('should render chart container when variant is "donut"', () => {
      const { container } = render(<LanguageChart data={mockData} variant="donut" />);
  ...
  describe('Language Grouping', () => {
    ...
    it('should show note about grouped languages', () => {
      ...
      expect(screen.getByText(/includes 2 additional languages/i)).toBeInTheDocument();
    ...
    it('should not group when maxLanguages is high enough', () => {
      ...
      expect(screen.queryByText(/includes.*additional/i)).not.toBeInTheDocument();
    });
  });
  ...
  describe('Single Language', () => {
    it('should handle single language correctly', () => {
      ...
      expect(screen.getByText(/1 language across repositories/i)).toBeInTheDocument();
      expect(screen.getByText('TypeScript')).toBeInTheDocument();
    });
  });
  ...
  describe('Loading State', () => {
    ...
    it('should show custom loading message', () => {
      ...
      expect(screen.getByText('Analyzing code languages...')).toBeInTheDocument();
    });

    it('should not show chart when loading', () => {
      render(<LanguageChart data={mockData} loading={true} />);

      expect(screen.queryByText('Language Distribution')).not.toBeInTheDocument();
    });
  });
  ...
  describe('Error State', () => {
    ...
    it('should show custom error title and description', () => {
      ...
      expect(screen.getByText('Unable to analyze languages')).toBeInTheDocument();
    });
  ...
  describe('Empty State', () => {
    it('should show empty state when data is empty array', () => {
      ...
      expect(screen.getByText('No programming language statistics available.')).toBeInTheDocument();
    });
  ...
  describe('State Priority', () => {
    it('should show loading state over error state', () => {
      ...
      expect(screen.getByText('Loading language statistics...')).toBeInTheDocument();
      expect(screen.queryByText('Failed to load language statistics')).not.toBeInTheDocument();
    });
  ...
  describe('Legend', () => {
    ...
    it('should render without legend when showLegend is false', () => {
      const { container } = render(<LanguageChart data={mockData} showLegend={false} />);
  ...
  describe('Language List Display', () => {
    ...
    it('should use singular "repo" for count of 1', () => {
      const singleRepoData: LanguageStats[] = [
        { name: 'TypeScript', size: 100000, percentage: 100, repositoryCount: 1 },
      ];
  ...
  describe('Edge Cases', () => {
    it('should handle two languages', () => {
      const twoLangs: LanguageStats[] = [
        { name: 'TypeScript', size: 600000, percentage: 60, repositoryCount: 8 },
        { name: 'JavaScript', size: 400000, percentage: 40, repositoryCount: 5 },
      ];
  ...
  describe('Accessibility', () => {
    ...
    it('should have heading for chart title', () => {
      ...
      const heading = screen.getByRole('heading', { name: /language distribution/i });
