...
describe("filterRepositories", () => {
  ...
  describe("Language filter", () => {
    it("should filter by primary language", () => {
      ...
      expect(filtered).toHaveLength(2);
      expect(
        filtered.every((r) => r.primaryLanguage?.name === "TypeScript"),
      ).toBe(true);
    });
  ...
  describe("Search query filter", () => {
    ...
    it("should be case-insensitive", () => {
      const repos = [createMockRepo({ id: "a", name: "TypeScript-Utility" })];
  ...
  describe("Topics filter", () => {
    it("should filter repos with topics", () => {
      ...
      expect(filtered).toHaveLength(2);
      expect(filtered.every((r) => r.repositoryTopics.nodes.length > 0)).toBe(
        true,
      );
    });
  });
