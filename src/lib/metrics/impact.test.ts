...
describe('calculateImpactScore', () => {
  ...
  describe('stars component (0-35 points)', () => {
    it('awards 35 points for 10000+ stars', () => {
      const timeline = [createYearData(2025, [createRepoContribution(10000, 0, 0)], [], 0, 0)];
    ...
    it('awards 30 points for 5000-9999 stars', () => {
      const timeline = [createYearData(2025, [createRepoContribution(5000, 0, 0)], [], 0, 0)];
      const result = calculateImpactScore(timeline);
      expect(result.breakdown.stars).toBe(30);
    });

    it('awards 25 points for 1000-4999 stars', () => {
      const timeline = [createYearData(2025, [createRepoContribution(1000, 0, 0)], [], 0, 0)];
      const result = calculateImpactScore(timeline);
      expect(result.breakdown.stars).toBe(25);
    });

    it('awards 20 points for 500-999 stars', () => {
      const timeline = [createYearData(2025, [createRepoContribution(500, 0, 0)], [], 0, 0)];
      const result = calculateImpactScore(timeline);
      expect(result.breakdown.stars).toBe(20);
    });

    it('awards 15 points for 100-499 stars', () => {
      const timeline = [createYearData(2025, [createRepoContribution(100, 0, 0)], [], 0, 0)];
      const result = calculateImpactScore(timeline);
      expect(result.breakdown.stars).toBe(15);
    });

    it('awards 10 points for 50-99 stars', () => {
      const timeline = [createYearData(2025, [createRepoContribution(50, 0, 0)], [], 0, 0)];
      const result = calculateImpactScore(timeline);
      expect(result.breakdown.stars).toBe(10);
    });

    it('awards 5 points for 10-49 stars', () => {
      const timeline = [createYearData(2025, [createRepoContribution(10, 0, 0)], [], 0, 0)];
      const result = calculateImpactScore(timeline);
      expect(result.breakdown.stars).toBe(5);
    });

    it('awards 0 points for < 10 stars', () => {
      const timeline = [createYearData(2025, [createRepoContribution(5, 0, 0)], [], 0, 0)];
      const result = calculateImpactScore(timeline);
    ...
    it('aggregates stars across multiple repos', () => {
      const timeline = [
        createYearData(
          2025,
          [createRepoContribution(100, 0, 0, 'repo1'), createRepoContribution(50, 0, 0, 'repo2')],
          [],
          0,
          0
        ),
  ...
  describe('forks component (0-20 points)', () => {
    it('awards 20 points for 1000+ forks', () => {
      const timeline = [createYearData(2025, [createRepoContribution(0, 1000, 0)], [], 0, 0)];
      const result = calculateImpactScore(timeline);
      expect(result.breakdown.forks).toBe(20);
    });

    it('awards 16 points for 500-999 forks', () => {
      const timeline = [createYearData(2025, [createRepoContribution(0, 500, 0)], [], 0, 0)];
      const result = calculateImpactScore(timeline);
      expect(result.breakdown.forks).toBe(16);
    });

    it('awards 12 points for 100-499 forks', () => {
      const timeline = [createYearData(2025, [createRepoContribution(0, 100, 0)], [], 0, 0)];
      const result = calculateImpactScore(timeline);
      expect(result.breakdown.forks).toBe(12);
    });

    it('awards 8 points for 50-99 forks', () => {
      const timeline = [createYearData(2025, [createRepoContribution(0, 50, 0)], [], 0, 0)];
      const result = calculateImpactScore(timeline);
      expect(result.breakdown.forks).toBe(8);
    });

    it('awards 4 points for 10-49 forks', () => {
      const timeline = [createYearData(2025, [createRepoContribution(0, 10, 0)], [], 0, 0)];
      const result = calculateImpactScore(timeline);
      expect(result.breakdown.forks).toBe(4);
    });

    it('awards 0 points for < 10 forks', () => {
      const timeline = [createYearData(2025, [createRepoContribution(0, 5, 0)], [], 0, 0)];
      const result = calculateImpactScore(timeline);
  ...
  describe('reach component (0-20 points)', () => {
    it('awards full 20 points for 500+ combined stars+forks', () => {
      const timeline = [createYearData(2025, [createRepoContribution(300, 200, 0)], [], 0, 0)];
      const result = calculateImpactScore(timeline);
      expect(result.breakdown.reach).toBe(20);
    });

    it('awards proportional points for lower reach', () => {
      const timeline = [createYearData(2025, [createRepoContribution(150, 100, 0)], [], 0, 0)];
      const result = calculateImpactScore(timeline);
      expect(result.breakdown.reach).toBe(10); // 250/500 * 20 = 10
    });

    it('caps at 20 points', () => {
      const timeline = [createYearData(2025, [createRepoContribution(10000, 5000, 0)], [], 0, 0)];
      const result = calculateImpactScore(timeline);
  ...
  describe('score calculation', () => {
    it('sums all components correctly', () => {
      const timeline = [createYearData(2025, [createRepoContribution(100, 50, 10)], [], 50, 50)];
      const result = calculateImpactScore(timeline);
    ...
    it('rounds score to nearest integer', () => {
      const timeline = [createYearData(2025, [createRepoContribution(15, 15, 5)], [], 30, 30)];
      const result = calculateImpactScore(timeline);
      expect(Number.isInteger(result.score)).toBe(true);
    });

    it('calculates exceptional impact correctly', () => {
      const timeline = [createYearData(2025, [createRepoContribution(10000, 1000, 500)], [], 150, 100)];
      const result = calculateImpactScore(timeline);
    ...
    it('calculates strong impact correctly', () => {
      const timeline = [createYearData(2025, [createRepoContribution(1000, 100, 50)], [], 80, 60)];
      const result = calculateImpactScore(timeline);
    ...
    it('calculates low impact correctly', () => {
      ...
      const timeline = [createYearData(2025, [createRepoContribution(50, 20, 5)], [], 30, 20)];
      const result = calculateImpactScore(timeline);
    ...
    it('calculates minimal impact correctly', () => {
      const timeline = [createYearData(2025, [createRepoContribution(5, 2, 1)], [], 5, 5)];
      const result = calculateImpactScore(timeline);
  ...
  describe('breakdown constraints', () => {
    it('ensures stars never exceeds 35', () => {
      const timeline = [createYearData(2025, [createRepoContribution(100000, 0, 0)], [], 0, 0)];
      const result = calculateImpactScore(timeline);
      expect(result.breakdown.stars).toBeLessThanOrEqual(35);
    });

    it('ensures forks never exceeds 20', () => {
      const timeline = [createYearData(2025, [createRepoContribution(0, 50000, 0)], [], 0, 0)];
      const result = calculateImpactScore(timeline);
      expect(result.breakdown.forks).toBeLessThanOrEqual(20);
    });

    it('ensures contributors never exceeds 15', () => {
      const timeline = [createYearData(2025, [createRepoContribution(0, 5000, 0)], [], 0, 0)];
      const result = calculateImpactScore(timeline);
      expect(result.breakdown.contributors).toBeLessThanOrEqual(15);
    });

    it('ensures reach never exceeds 20', () => {
      const timeline = [createYearData(2025, [createRepoContribution(50000, 50000, 0)], [], 0, 0)];
      const result = calculateImpactScore(timeline);
      expect(result.breakdown.reach).toBeLessThanOrEqual(20);
    });
    ...
    it('ensures total score never exceeds 100', () => {
      const timeline = [createYearData(2025, [createRepoContribution(100000, 50000, 10000)], [], 1000, 1000)];
      const result = calculateImpactScore(timeline);
