...
test.describe('Cache Transition: Demo → Auth', () => {
  test('should fetch fresh data after OAuth login', async ({ page, context }) => {
    ...
    const apiCalls = requests.filter(req => req.url().includes('/api/github-proxy'))
...
test.describe('Session Expiration Mid-Use', () => {
  test('should notify user and fallback to demo mode when session expires', async ({ page, context }) => {
    // 1. Authenticate user
    await page.goto('/')
    await page.click('button:has-text("Sign in with GitHub")')
    await page.waitForURL('/?auth=success')
  ...
  test('should handle session expiration without breaking app', async ({ page }) => {
    // 1. Authenticate
