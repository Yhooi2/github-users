...
describe('OAuth Analytics Logger', () => {
  ...
  describe('logOAuthLogin', () => {
    ...
    it('должен пропустить логирование если KV недоступен', async () => {
      ...
      const { logOAuthLogin: logOAuthLoginWithNullKv } = await import('./logger')
      ...
      expect(console.warn).toHaveBeenCalledWith('KV not available, skipping OAuth login logging')
    })
  ...
  describe('logOAuthLogout', () => {
    it('должен логировать OAuth logout в KV', async () => {
      ...
      // Проверяем console.log
      expect(console.log).toHaveBeenCalledWith('OAuth logout logged: octocat (12345)')
    })
    ...
    it('должен пропустить логирование если KV недоступен', async () => {
      ...
      // Re-import logger with null kv
      const { logOAuthLogout: logOAuthLogoutWithNullKv } = await import('./logger')
      ...
      expect(console.warn).toHaveBeenCalledWith('KV not available, skipping OAuth logout logging')
    })
  ...
  describe('logRateLimitSnapshot', () => {
    ...
    it('должен пропустить логирование если KV недоступен (без warning)', async () => {
      ...
      // Re-import logger with null kv
      const { logRateLimitSnapshot: logRateLimitSnapshotWithNullKv } = await import('./logger')
    ...
    it('должен обработать ошибку при zadd', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Failed to log rate limit snapshot:', error)
    })
  })
  ...
  describe('updateSessionActivity', () => {
    ...
    it('должен пропустить если KV недоступен', async () => {
      ...
      // Re-import logger with null kv
      const { updateSessionActivity: updateSessionActivityWithNullKv } = await import('./logger')
    ...
    it('должен обработать ошибку при get', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Failed to update session activity:', error)
    })
  ...
  describe('cleanupOldAnalytics', () => {
    it('должен удалить старые login events (>30 дней)', async () => {
      ...
      expect(kv.zremrangebyscore).toHaveBeenCalledWith('analytics:oauth:logins', 0, thirtyDaysAgo)
    })
    ...
    it('должен удалить старые logout events (>30 дней)', async () => {
      ...
      expect(kv.zremrangebyscore).toHaveBeenCalledWith('analytics:oauth:logouts', 0, thirtyDaysAgo)
    })
    ...
    it('должен удалить старые rate limit snapshots (>7 дней)', async () => {
      ...
      expect(kv.zremrangebyscore).toHaveBeenCalledWith('analytics:ratelimit', 0, sevenDaysAgo)
    })
    ...
    it('должен пропустить если KV недоступен', async () => {
      ...
      // Re-import logger with null kv
      const { cleanupOldAnalytics: cleanupOldAnalyticsWithNullKv } = await import('./logger')
    ...
    it('должен обработать ошибку при cleanup', async () => {
      ...
      expect(console.error).toHaveBeenCalledWith('Failed to cleanup old analytics:', error)
    })
  })
})
