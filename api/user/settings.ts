...
const DEFAULT_SETTINGS: Omit<
  UserSettings,
  "userId" | "login" | "createdAt" | "updatedAt"
> = {
  preferences: {
  ...
  try {
    ...
    if (req.method === "PUT" || req.method === "PATCH") {
      ...
      if (
        ...
        (updatedPreferences.itemsPerPage < 1 ||
          updatedPreferences.itemsPerPage > 100)
      ) {
        return res.status(400).json({
          error: "Bad request",
      ...
      if (
        ...
        (updatedPreferences.refreshInterval < 5000 ||
          updatedPreferences.refreshInterval > 300000)
      ) {
        return res.status(400).json({
          error: "Bad request",
          message:
            "Invalid refreshInterval. Must be between 5000 (5s) and 300000 (5min)",
        });
      }
