...
describe('OAuth Login Endpoint', () => {
  ...
  it('должен редиректить на GitHub с правильными параметрами', async () => {
    ...
    expect(res.setHeader).toHaveBeenCalledWith(
      'Set-Cookie',
      expect.stringMatching(/^oauth_state=.+; HttpOnly; Secure; SameSite=Lax; Max-Age=600; Path=\/$/)
    )
  ...
  it('должен использовать localhost для development', async () => {
    ...
    const redirectCall = (res.redirect as ReturnType<typeof vi.fn>).mock.calls[0][0]
    expect(redirectCall).toContain('redirect_uri=http%3A%2F%2Flocalhost%3A3000%2Fapi%2Fauth%2Fcallback')
  })
  ...
  it('должен использовать VERCEL_URL для production', async () => {
    ...
    const redirectCall = (res.redirect as ReturnType<typeof vi.fn>).mock.calls[0][0]
    expect(redirectCall).toContain('redirect_uri=https%3A%2F%2Fmy-app.vercel.app%2Fapi%2Fauth%2Fcallback')
  })
  ...
  it('должен генерировать уникальный state каждый раз', async () => {
    ...
    // Первый вызов
    await handler(req as VercelRequest, res as VercelResponse)
    const firstCookie = (res.setHeader as ReturnType<typeof vi.fn>).mock.calls[0][1]
    ...
    // Второй вызов
    await handler(req as VercelRequest, res as VercelResponse)
    const secondCookie = (res.setHeader as ReturnType<typeof vi.fn>).mock.calls[0][1]
