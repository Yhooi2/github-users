...
test.describe("User Analytics Flow", () => {
  test("complete analytics flow from search to timeline", async ({ page }) => {
    ...
    if (yearExists) {
      ...
      const expandedContent = page.locator(
        "text=/Your Projects|Open Source Contributions|commits/i",
      );
  ...
  test("handles user not found", async ({ page }) => {
    ...
    // Check error message
    await expect(
      page.locator("text=/User not found|Error|failed to fetch/i"),
    ).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test("displays contribution history", async ({ page }) => {
    ...
    const activitySection = page.locator(
      "text=/Recent Activity|Contribution History|Activity Timeline/i",
    );
