...
test.describe("Real API Integration E2E", () => {
  ...
  test("should successfully search and display user profile from real GitHub API", async ({
    page,
  }) => {
  ...
  test("should make GraphQL requests to correct endpoint", async ({ page }) => {
    ...
    const hasProxyRequest = apiRequests.some((url) =>
      url.includes("/api/github-proxy"),
    );
  ...
  test("should verify Apollo Client works with real responses", async ({
    page,
  }) => {
    ...
    // Ожидаем результата
    await page.waitForLoadState("networkidle");
    await expect(page.locator("text=Dan Abramov")).toBeVisible({
      timeout: 10000,
    });
    ...
    console.log(
      `✅ Apollo Client integration verified (${graphqlRequestsCount} requests)`,
    );
  });
  ...
  test("should cache results (no duplicate requests for same user)", async ({
    page,
  }) => {
    ...
    console.log(
      `Cache behavior: ${newRequests === 0 ? "✅ Cache HIT" : "⚠️ Cache MISS or disabled"}`,
    );
  });
});
...
test.describe("Rate Limit Banner E2E", () => {
  ...
  test("should NOT display rate limit banner when rate limit is high (normal case)", async ({
    page,
  }) => {
    await page.goto("/");
    ...
    const rateLimitAlert = page.locator(
      '[role="alert"]:has-text("requests remaining")',
    );
  ...
  test("should display user profile even when rate limit banner is hidden", async ({
    page,
  }) => {
    ...
    console.log(
      "✅ User profile displays correctly regardless of rate limit banner visibility",
    );
  });
});
...
test.describe("Error Handling E2E", () => {
  test("should display error state when user not found", async ({ page }) => {
    ...
    await page.fill(
      ...
      "thisuserdoesnotexist123456789xyz",
    );
    await page.click('button:has-text("Search")');
