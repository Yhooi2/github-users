...
test.describe("Cache Transition: Demo → Auth", () => {
  test("should fetch fresh data after OAuth login (not use demo cache)", async ({
    ...
    const _requests = await setupRequestTracking(page);
    ...
    expect(
      _requests[0].rateLimit.isDemo,
      "First request should be in demo mode",
    ).toBe(true);

    expect(
      _requests[0].rateLimit.remaining,
  ...
  test("should make multiple API calls when transitioning demo → auth (no cache reuse)", async ({
    ...
    expect(demoRequestCount, "Should have made 1 request in demo mode").toBe(1);
  ...
  test("should display authenticated banner after cache transition", async ({
    page,
    context,
  }) => {
  ...
  test("should handle cache transition with different users", async ({
    page,
    context,
  }) => {
    const requests = await setupRequestTracking(page);
