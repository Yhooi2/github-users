...
test.describe("OAuth Authentication Flow", () => {
  ...
  test('should show "Sign in with GitHub" button when not authenticated', async ({
    page,
  }) => {
  ...
  test("should redirect to GitHub OAuth on sign in click", async ({ page }) => {
    ...
    await page.route("/api/auth/login", async (route) => {
      ...
      await route.fulfill({
        ...
        headers: {
          Location:
            "https://github.com/login/oauth/authorize?client_id=test&state=mock_state",
        },
      });
    });
  ...
  test("should complete OAuth login flow successfully", async ({
    page,
    context,
  }) => {
  ...
  test("should display authenticated user information in dropdown", async ({
    page,
    context,
  }) => {
  ...
  test("should complete logout flow successfully", async ({
    ...
    const avatarAfterLogout = page
      .locator('[role="button"]')
      .filter({ has: page.locator('img[alt*="@"]') });
    await expect(avatarAfterLogout).not.toBeVisible();
  });

  test("should show demo mode banner when not authenticated", async ({
    page,
  }) => {
  ...
  test("should show authenticated banner when OAuth authenticated", async ({
    page,
    context,
  }) => {
    // Set session cookie
  ...
  test("should upgrade from demo to OAuth seamlessly", async ({
    page,
    context,
  }) => {
    // Start in demo mode
    ...
    // Verify demo mode
    let signInButton = page.getByRole("button", {
      name: /sign in with github/i,
    });
    await expect(signInButton).toBeVisible();
